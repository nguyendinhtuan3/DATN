...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        const [existingEmails] = await conn.query('SELECT id FROM users WHERE email = ?', [email]);
        if (existingEmails.length > 0) {
            return res.json({ status: false, message: 'Email đã được sử dụng' });
        }

        const [existingNames] = await conn.query('SELECT id FROM users WHERE username = ?', [name]);
...
router.post('/reset-password', async (req, res) => {
    ...
    try {
        if (!token || !newPassword) {
            return res.status(400).json({ status: false, message: 'Thiếu token hoặc mật khẩu' });
        }
        ...
        const conn = db.promise();
        await conn.query('UPDATE users SET password = ? WHERE id = ?', [hashed, payload.id]);
...
router.put('/users/:id', async (req, res) => {
    ...
    try {
        ...
        if (updateFields.length === 0) {
            return res.status(400).json({ status: false, message: 'Không có dữ liệu cập nhật' });
        }
        ...
        const [updatedUsers] = await conn.query('SELECT id, username, email, avatar, role FROM users WHERE id = ?', [
            userId,
        ]);
...
router.get('/users', verifyRole('Admin'), async (req, res) => {
    try {
        const conn = db.promise();
        const [users] = await conn.query('SELECT id, username, email, role FROM users');
...
router.post('/assign-role', verifyRole('Admin'), async (req, res) => {
    ...
    if (!['Admin', 'User', 'Giảng viên'].includes(role)) {
        return res.status(400).json({ status: false, message: 'Vai trò không hợp lệ' });
    }
