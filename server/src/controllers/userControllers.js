...
// 📌 Lấy danh sách người dùng (tìm kiếm & phân trang, loại trừ người đang đăng nhập)
router.get('/all', verifyRole('admin'), async (req, res) => {
    const { search = '', page = 1, limit = 10 } = req.query;
    const offset = (page - 1) * limit;

    const currentUserId = req.user?.id; // lấy id người dùng hiện tại
    ...
    try {
       ...
       ORDER BY id DESC
       LIMIT ? OFFSET ?`,
            [`%${search}%`, `%${search}%`, currentUserId, parseInt(limit), parseInt(offset)],
        );
...
router.get('/:id/detail', verifyRole('admin'), async (req, res) => {
    ...
    try {
        const conn = db.promise();
        const [users] = await conn.query('SELECT id, username, email, role FROM users WHERE id = ?', [id]);
...
router.put('/:id/update', verifyRole('admin'), async (req, res) => {
    ...
    try {
        ...
        await conn.query(`UPDATE users SET ${updateFields.join(', ')} WHERE id = ?`, updateValues);
