...
import { getUserDetail, getUserProfile } from "./api/userApi";

const App = () => {
  const { user, setUser, isUserLoggedIn } = useAuthStore();
  useEffect(() => {
            ...
            <>
              ...
              {user.role === "admin" ? (
                <>
                  ...
                  />
                  <Route
                    path="/admin/frame-vocabulary"
                    element={<FrameVocabularyPage />}
                  />
                  <Route path="/admin/floor1" element={<Floor1Manager />} />
                  <Route
                    path="/admin/floor2"
                    element={<Floor2MazePopupPage />}
                  />
                  <Route
                    path="/admin/floor3"
                    element={<Floor3ListenChoosePage />}
                  />
                </>
              ) : user.role === "teacher" ? (
                <>
