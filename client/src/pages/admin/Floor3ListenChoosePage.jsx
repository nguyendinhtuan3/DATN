...
} from '../../api/floorApi'; // Đảm bảo đúng đường dẫn
...
const Floor3ListenChoosePage = () => {
    ...
    const validateForm = () => {
        const { question, options, correct_answer } = formData;
        if (!question.trim()) return setValidationError('Câu hỏi không được để trống.');
        if (options.length !== 3 || options.some((opt) => !opt.trim())) {
            return setValidationError('Phải nhập đủ 4 lựa chọn.');
        }
        if (!correct_answer.trim() || !options.includes(correct_answer)) {
            return setValidationError('Đáp án đúng phải trùng với một trong 4 lựa chọn.');
        }
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!validateForm()) return showNotification(validationError, 'error');
        setIsLoading(true);
        try {
            const payload = {
                ...formData,
    ...
    return (
        <div className="flex">
            ...
            <div className="flex-1 p-6">
                ...
                <button onClick={handleAddNewClick} className="mb-4 px-4 py-2 bg-blue-600 text-white rounded">
                    Thêm mới
                </button>
                ...
                <ul>
                    {filteredQuestions.map((q) => (
                        <li key={q.id} className="mb-4 p-4 border rounded shadow">
                            ...
                            <p className="text-green-600 mt-1">Đáp án đúng: {q.correct_answer}</p>
