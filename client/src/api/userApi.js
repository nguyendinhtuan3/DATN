...
export const getUserDetail = async (userId) => {
    const response = await authClient.get(`${API_URL}/api/users/${userId}/detail`);
    return response.data;
};
...
export const updateUserByAdmin = async (userId, formData) => {
    const response = await authClient.put(`${API_URL}/api/users/${userId}/update`, formData);
    return response.data;
};

// 🗑️ Xoá người dùng
export const deleteUser = async (userId) => {
    const response = await authClient.delete(`${API_URL}/api/users/${userId}/delete`);
    return response.data;
};