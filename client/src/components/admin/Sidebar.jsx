...
import { FaUsers, FaBook, FaGift, FaMoneyBill, FaBullhorn, FaBars } from 'react-icons/fa';
...
const Sidebar = () => {
    ...
    return (
        <div
            className={`${collapsed ? 'w-16' : 'w-64'} bg-white border-r shadow-sm h-full transition-all duration-300`}
        >
            {/* Header with toggle */}
            <div className="flex items-center justify-between p-4 border-b">
                {!collapsed && <div className="font-bold text-lg whitespace-nowrap">Admin Panel</div>}
                <button onClick={() => setCollapsed(!collapsed)} className="text-gray-600 hover:text-blue-600">
                    <FaBars />
                </button>
            </div>
